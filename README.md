...
- `[RAG]` / `[標準検索]` バッジ表示
- WordPress AI Client 経由の埋め込み生成を優先し、OpenAI / Azure OpenAI fallback とローカルまたは self-hosted provider に対応
...
WordPress 7.0 の AI Client が埋め込み生成と `text-embedding-3-large` の選択に対応している場合は、サイト全体の AI 設定と API Key を利用します。埋め込みモデルを選択できない場合は、OpenAI fallback または Azure OpenAI fallback を選んでください。
...
- WordPress AI Client を優先しますが、現在の core/provider 構成で embedding 生成や `text-embedding-3-large` の選択ができない場合は OpenAI / Azure OpenAI fallback を使ってください。
