...
AI Retriever は、WordPress の検索に RAG 風のベクトル検索を追加する実験的なプラグインです。
...
- `[RAG]` / `[標準検索]` バッジ表示
- 外部 provider は OpenAI / Azure OpenAI、ローカルまたは self-hosted provider は Ollama / LM Studio / Infinity / TEI / Custom HTTP に対応
...
1. 配布 ZIP `ai-retriever-0.2.0.zip` を用意します。
2. WordPress 管理画面で `プラグイン -> 新規追加 -> プラグインのアップロード` を開きます。
3. ZIP をアップロードします。
4. `AI Retriever` を有効化します。
5. `設定 -> AI Retriever` を開きます。

### 手動インストール

1. `ai-retriever` ディレクトリを `wp-content/plugins/` に配置します。
2. WordPress 管理画面の `プラグイン` で `AI Retriever` を有効化します。
3. `設定 -> AI Retriever` を開きます。

## 初期設定の流れ

`設定 -> AI Retriever` は、作業順に番号付きで並んでいます。
...
- `dist/ai-retriever-0.2.0.zip`

## 既知の制限

- MariaDB 11.7 以上を主な対象にしています。
- MySQL native vector 対応は既定では無効です。
- 外部 provider は OpenAI / Azure OpenAI に限定しています。Anthropic Claude / Google Gemini / Imagen / OpenAI GPT / DALL-E などの WordPress コア LLM API と API キーが統合されても、プラグインから安全に embedding API として使える前提にはしていません。
